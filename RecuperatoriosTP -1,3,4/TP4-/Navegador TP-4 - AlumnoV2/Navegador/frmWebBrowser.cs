...
namespace Navegador
{
    public partial class frmWebBrowser : Form
    {
        ...
        {
            //BARRA DE PROGRESO VUELVE A 0
            this.tspbProgreso.Value = 0;


            //if (!this.txtUrl.Text.Contains("html://"))
            //{
            //    this.txtUrl.Text = this.txtUrl.Text.Insert(0,"html://");
            //}

            try
            {
                Uri direccion = new Uri(this.txtUrl.Text, UriKind.Absolute);

                Descargador dw = new Descargador(direccion);
                dw.eventProgress += new Descargador.EventProgress(this.ProgresoDescarga);
                dw.eventCompleted += new Descargador.EventCompleted(this.FinDescarga);


                Thread hilo = new Thread(new ThreadStart(dw.IniciarDescarga));
                hilo.Start();
            }
